...
import { 
  MessageSquare, Clock, User as UserIcon, Plus, RotateCw, X, 
...
const tareasFiltradas = computed(() => {
  return tareas.value.filter((t) => {
    if (fEjecutor.value && String(t.asignada_a) !== String(fEjecutor.value)) return false;
    return true;
  });
});

// Update columns when tareasFiltradas change
...
async function cargarTodo() {
  ...
  cargando.value = true;
  try {
    const [execs, tasks] = await Promise.all([
      api.get("users/executors/").then((r) => r.data),
      api.get("tasks/").then((r) => r.data),
    ]);
    ejecutores.value = execs || [];
    tareas.value = tasks || [];
  } catch (e) {
    err.value = e?.response?.data?.detail || "No se pudo cargar el panel admin.";
  } finally {
...
async function recargarTareas() {
  ...
  try {
    ...
    err.value = "No se pudieron cargar las tareas.";
  } finally {
    cargando.value = false;
  }
}
...
const form = ref({ titulo: "", descripcion: "", asignada_a: "", estado: "PENDIENTE" });

function abrirCrear() {
  form.value = { titulo: "", descripcion: "", asignada_a: "", estado: "PENDIENTE" };
...
async function crearTarea() {
  try {
    await api.post("tasks/", {
      ...form.value,
      asignada_a: form.value.asignada_a || null,
    });
    showCrear.value = false;
    await recargarTareas();
  } catch (e) {
    err.value = "No se pudo crear la tarea.";
  }
}
...
async function reasignar(nuevoAsignadoId) {
  ...
  const oldId = tareaDetalle.value.asignada_a;
  tareaDetalle.value.asignada_a = nuevoAsignadoId || null;
  const tLocal = tareas.value.find(t => t.id === tareaDetalle.value.id);
  if (tLocal) tLocal.asignada_a = nuevoAsignadoId || null;

  try {
    await api.patch(`tasks/${tareaDetalle.value.id}/`, { asignada_a: nuevoAsignadoId || null });
    await recargarTareas(); // Refresh usernames
  } catch (e) {
    tareaDetalle.value.asignada_a = oldId;
    if (tLocal) tLocal.asignada_a = oldId;
  }
}
...
<template>
  <div class="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div class="flex items-center gap-6">
        <div class="flex gap-4">
          <div class="text-center">
            <div class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total</div>
        ...
        <div class="hidden md:flex items-center gap-2">
          <span class="text-xs text-slate-500 font-medium">Ejecutor:</span>
          <select v-model="fEjecutor" class="bg-[#1E2028] border border-white/5 rounded-md text-xs text-slate-200 p-1.5 focus:outline-none focus:border-blue-500/50">
            <option value="">Todos</option>
            <option v-for="u in ejecutores" :key="u.id" :value="u.id">{{ u.username }}</option>
          </select>
        </div>
      </div>

      <div class="flex items-center gap-3 mt-4 md:mt-0">
        <button @click="recargarTareas" class="p-2 text-slate-400 hover:text-white bg-[#1E2028] rounded-lg border border-white/5 transition-colors" title="Refrescar">
          <RotateCw class="w-4 h-4" :class="{ 'animate-spin': cargando }" />
        </button>
        <button @click="abrirCrear" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors shadow-[0_0_15px_rgba(37,99,235,0.3)]">
          <Plus class="w-4 h-4" />
          Nueva Tarea
        </button>
      </div>
              ...
              <div @click="abrirDetalles(element)" class="bg-[#1E2028] border border-white/5 p-4 rounded-lg cursor-pointer hover:border-slate-600 transition-colors shadow-sm group">
                ...
                <div class="mt-4 flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="w-5 h-5 rounded-full bg-slate-700 flex items-center justify-center text-[9px] font-bold text-white uppercase">
                      {{ element.asignada_a_username ? element.asignada_a_username[0] : '?' }}
                    </div>
                    <span class="text-[10px] text-slate-400">{{ element.asignada_a_username || 'Sin asignar' }}</span>
                  </div>
          ...
          >
            <template #item="{ element }">
              <div @click="abrirDetalles(element)" class="bg-[#1E2028] border border-blue-500/20 p-4 rounded-lg cursor-pointer hover:border-blue-500/50 transition-colors shadow-sm group">
                ...
                <div class="mt-4 flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="w-5 h-5 rounded-full bg-blue-900/50 text-blue-200 flex items-center justify-center text-[9px] font-bold uppercase ring-1 ring-blue-500/30">
                      {{ element.asignada_a_username ? element.asignada_a_username[0] : '?' }}
                    </div>
                    <span class="text-[10px] text-slate-400">{{ element.asignada_a_username || 'Sin asignar' }}</span>
                  </div>
          ...
          >
            <template #item="{ element }">
              <div @click="abrirDetalles(element)" class="bg-[#1E2028] border border-emerald-500/20 opacity-80 p-4 rounded-lg cursor-pointer hover:opacity-100 hover:border-emerald-500/40 transition-all shadow-sm group">
                ...
                <div class="mt-4 flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="w-5 h-5 rounded-full bg-emerald-900/30 text-emerald-300 flex items-center justify-center text-[9px] font-bold uppercase ring-1 ring-emerald-500/30">
                      {{ element.asignada_a_username ? element.asignada_a_username[0] : '?' }}
                    </div>
                  </div>
          ...
          <div class="bg-[#15161A] border border-white/5 rounded-xl p-4 space-y-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2 text-sm text-slate-400">
                <UserIcon class="w-4 h-4" /> Ejecutor
              </div>
              <select 
                :value="tareaDetalle.asignada_a" 
                @change="reasignar($event.target.value)"
                class="bg-[#1E2028] border border-white/10 text-sm text-white rounded-lg p-1.5 focus:outline-none focus:border-blue-500/50 min-w-[140px]"
              >
    ...
    <!-- Modal Nueva Tarea -->
    <div v-if="showCrear" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div class="bg-[#0B0C10] border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <h2 class="text-xl font-bold text-white mb-1">Nueva Tarea</h2>
        <p class="text-sm text-slate-400 mb-6">Crea una nueva tarea y asígnala al equipo.</p>
        
        <div class="space-y-4">
          <div>
            <label class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Título</label>
            <input v-model="form.titulo" type="text" class="w-full bg-[#1E2028] border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500/50 focus:outline-none" placeholder="Ej. Actualizar diseño de DB" />
          </div>
          <div>
            <label class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Descripción</label>
            <textarea v-model="form.descripcion" rows="3" class="w-full bg-[#1E2028] border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500/50 focus:outline-none" placeholder="Detalles..."></textarea>
          </div>
          <div>
            <label class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Asignar a</label>
            <select v-model="form.asignada_a" class="w-full bg-[#1E2028] border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500/50 focus:outline-none">
              <option value="">Sin asignar</option>
              <option v-for="u in ejecutores" :key="u.id" :value="u.id">{{ u.username }}</option>
            </select>
          </div>
        ...
        <div class="mt-8 flex justify-end gap-3">
          <button @click="showCrear = false" class="px-4 py-2 text-sm font-medium text-slate-300 hover:text-white transition-colors">Cancelar</button>
          <button @click="crearTarea" :disabled="!form.titulo" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 text-white text-sm font-medium rounded-lg transition-colors shadow-lg shadow-blue-500/20">
            Crear Tarea
          </button>
        </div>
      </div>
